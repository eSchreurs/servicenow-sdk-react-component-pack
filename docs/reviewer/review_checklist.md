...
- [ ] Both `servicenow-app` and `npm-package` exist at the root level
- [ ] Both have `now.config.json`, `now.prebuild.mjs`, `package.json`, `.eslintrc`
- [ ] `servicenow-app` scope is `x_est_react_pack`
- [ ] `npm-package` has correct folder structure: `components/atoms`, `components/molecules`, `components/organisms`, `services/`, `context/`, `theme/`, `types/`
- [ ] All service files exist as placeholders: `CacheService`, `ServiceNowClient`, `MetadataService`, `RecordService`, `SearchService`, `RhinoService`
- [ ] All context files exist as placeholders: `ThemeContext`, `ServiceNowContext`
- [ ] `theme/theme.ts` and `types/index.ts` exist as placeholders
- [ ] No business logic implemented anywhere — placeholders only
- [ ] No third-party packages in either `package.json`
